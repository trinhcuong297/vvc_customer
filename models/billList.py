...
class WaterFeeTable(models.Model):
    ...
    oldWater = fields.Many2one(string="Phiếu nước tháng trước", comodel_name="customer.qr", domain=[('type','=','water'),('status','=','ok'),('qrCodeScan','=',ground_ids.waterClockQrCode)])
    oldNumWater = fields.Float(string = "Chỉ số nước trước(m3)", related='oldWater.water')
    newWater = fields.Many2one(string="Phiếu nước tháng này", comodel_name="customer.qr", domain=[('type','=','water'),('status','=','ok'),('qrCodeScan','=',ground_ids.waterClockQrCode)])
